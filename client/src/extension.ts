...
import { GoalsPanel, ProofState, Goal, Hypothesis } from "./GoalsPanel";

let goalsPanel: GoalsPanel | undefined;

type Formula =
  | { kind: "atom"; name: string; negated: boolean }
  | { kind: "tensor" | "with" | "plus" | "lolli"; left: Formula; right: Formula }
  | { kind: "bang"; of: Formula }
  | { kind: "one" | "bot" | "top" | "zero" };

type GoalNode = {
  id: string;
  ctx: InternalHypothesis[];
  target: Formula;
};

type InternalHypothesis = Hypothesis & { formula: Formula };
type Completion = {
  label: string;
  insertText: string;
  detail?: string;
...
function textUpToCursor(editor: vscode.TextEditor): string {
...
function ensureGoalsPanel(
  ...
  if (!goalsPanel) {
    goalsPanel = GoalsPanel.createOrShow(
      context,
 ...
 */
function computeProofStateFromText(text: string): ProofState {
  ...
  const startNewTheorem = (name?: string) => {
    flushEngine();
    theoremCounter += 1;
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.onDidChangeTextEditorSelection((e) => {
      if (e.textEditor.document.languageId !== "mypa") return;
      scheduleUpdate();
    })
  );
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeTextDocument((e) => {
      if (e.document.languageId !== "mypa") return;
      scheduleUpdate();
    })
  );
          ...
          for (let line = 0; line < doc.lineCount; line++) {
            ...
            if (trimmed.startsWith("--")) {
              builder.push(
                ...
                0,
                text.length,
                semanticTokenTypes.indexOf("comment"),
                0
              );
              continue;
            }
            ...
            while ((m = operatorRe.exec(text))) {
              builder.push(
                ...
                m.index,
                1,
                semanticTokenTypes.indexOf("operator"),
                0
              );
            }
          }
...
class FormulaParser {
  ...
  parse(): Formula {
    ...
    if (this.pos !== this.input.length) {
      throw new Error(
        `Unexpected trailing input "${this.input.slice(this.pos)}"`
      );
    }
    return result;
  }
  ...
  private parsePrimary(): Formula {
    ...
    if (this.matchSymbol("1") || this.matchWord("one")) return { kind: "one" };
    if (this.matchSymbol("⊤") || this.matchWord("top")) return { kind: "top" };
    ...
    if (this.matchSymbol("⊥") || this.matchWord("bot")) return { kind: "bot" };
...
function renderFormula(f: Formula, parentPrec = 0): string {
  const paren = (prec: number, inner: string) =>
    prec < parentPrec ? `(${inner})` : inner;
  switch (f.kind) {
    case "atom":
...
function cloneCtx(ctx: InternalHypothesis[]): InternalHypothesis[] {
  return ctx.map((h) => ({ ...h }));
}
...
class ProofEngine {
  ...
  addError(line: number, msg: string) {
    this.errors.push(`Line ${line}: ${msg}`);
  }

  addHyp(name: string, formula: Formula) {
    const hyp: InternalHypothesis = {
      name,
  ...
  applyTactic(name: string, rawArgs: string, line: number) {
    ...
    const putBack = (newGoals: GoalNode[]) => {
      this.goals = newGoals.concat(this.goals);
    };

    const ensureHyp = (hypName?: string): InternalHypothesis | undefined => {
      if (!hypName) return ctx.find((h) => formulaEquals(h.formula, target));
    ...
    const closeWithHyp = (hypName?: string) => {
    ...
    const destructHyp = (hName: string, choose?: "left" | "right", extra?: string[]) => {
      ...
      const makeFreshName = (existing: InternalHypothesis[], base: string) => {
        let suffix = 1;
      ...
      if (hyp.formula.kind === "with") {
        ...
        if (!branchChoice && extra && extra.length > 0) {
          ...
          if (tok === "left" || tok === "right") branchChoice = tok as "left" | "right";
        }
        if (!branchChoice) {
      ...
      this.addError(
        line,
        `destruct/cases not supported for hypothesis "${hName}".`
      );
      this.goals.unshift(goal);
    };
        ...
        try {
          ...
          if (!parsed) {
            ...
            try {
              ...
              this.addError(line, (err as Error).message);
              this.goals.unshift(goal);
              return;
            }
          }
  ...
  finalize(): ProofState {
    ...
    const goals: Goal[] = this.goals.map((g) => ({
      id: g.id,
